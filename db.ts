...
// Query result type — compatible with existing call sites
  ...
  query: async (sql: string, params: any[] = []): Promise<QueryResult> => {
   ...
   try {
      ...
      if (err.message?.includes('UNIQUE constraint failed')) {
        ...
        if (match) err.constraint = match[1];
      }
      throw err;
    }
  },
...
init();
  ...
  try { database.exec(`ALTER TABLE users ADD COLUMN avatar_color TEXT NOT NULL DEFAULT '#6366f1'`); } catch { /* już istnieje */ }
  try { database.exec(`ALTER TABLE messages ADD COLUMN deleted_at DATETIME`); } catch { /* już istnieje */ }
  try { database.exec(`ALTER TABLE messages ADD COLUMN edited_at DATETIME`); } catch { /* już istnieje */ }
  try { database.exec(`ALTER TABLE personal_messages ADD COLUMN deleted_at DATETIME`); } catch { /* już istnieje */ }
  try { database.exec(`ALTER TABLE personal_messages ADD COLUMN edited_at DATETIME`); } catch { /* już istnieje */ }
