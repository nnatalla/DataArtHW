...
interface AuthRequest extends Request {
  user?:  { id: number; email: string; username: string; avatar_color: string };
  token?: string;
}
...
async function authenticateToken(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    if (!userResult.rows.length) { res.status(403).json({ error: 'User not found' }); return; }
    ...
    if (!sessionResult.rows.length) { res.status(403).json({ error: 'Session expired' }); return; }
...
function addUserSocket(userId: number, socketId: string): void {
...
function removeUserSocket(userId: number, socketId: string): void {
...
function getUserStatus(userId: number): 'offline' | 'afk' | 'online' {
  const sockets = userToSockets.get(userId);
...
async function broadcastPresence(userId: number): Promise<void> {
...
setInterval(() => {
  for (const [userId] of userToSockets) broadcastPresence(userId);
}, 30_000);

// ─── Query Helpers ────────────────────────────────────────────
async function buildRoomMessage(messageId: number) {
  const result = await pool.query(
...
async function buildPersonalMessage(messageId: number) {
  const result = await pool.query(
...
async function isUserBlocked(userId: number, otherId: number): Promise<boolean> {
...
async function canMessageUser(userId: number, otherId: number): Promise<boolean> {
  if (await isUserBlocked(userId, otherId)) return false;
  return areFriends(userId, otherId);
}

// ─── Socket.io ────────────────────────────────────────────────
type AppSocket = Socket & { userId?: number };
...
io.on('connection', (socket: AppSocket) => {
  ...
  socket.on('authenticate', async (token: string) => {
    try {
      ...
      const rooms = await pool.query('SELECT room_id FROM room_members WHERE user_id = $1', [decoded.userId]);
      rooms.rows.forEach((r: any) => socket.join('room_' + r.room_id));
  ...
  socket.on('join_room',  (roomId: number) => { if (socket.userId) socket.join('room_'  + roomId); });
  socket.on('leave_room', (roomId: number) => { if (socket.userId) socket.leave('room_' + roomId); });
  ...
  socket.on('send_personal_message', async (data: any) => {
    if (!socket.userId) return;
  ...
  socket.on('typing', (data: any) => {
    if (!socket.userId) return;
    if (data.roomId)     socket.to('room_' + data.roomId).emit('typing', { userId: socket.userId, username: data.username, roomId: data.roomId });
    else if (data.receiverId) emitToUser(data.receiverId, 'typing', { userId: socket.userId, username: data.username });
  });

  socket.on('stop_typing', (data: any) => {
    if (!socket.userId) return;
    if (data.roomId)     socket.to('room_' + data.roomId).emit('stop_typing', { userId: socket.userId });
    else if (data.receiverId) emitToUser(data.receiverId, 'stop_typing', { userId: socket.userId });
  });

  socket.on('disconnect', () => {
    if (socket.userId) { removeUserSocket(socket.userId, socket.id); broadcastPresence(socket.userId); }
  });
});
...
app.put('/api/auth/password', authenticateToken, async (req: AuthRequest, res: Response) => {
...
app.delete('/api/auth/account', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const ownedRooms = await pool.query('SELECT id FROM rooms WHERE owner_id=$1', [req.user!.id]);
    ...
    res.json({ ok: true });
  } catch (err) { console.error('delete account error:', err); res.status(500).json({ error: 'Failed to delete account' }); }
});

// ─── Sessions ─────────────────────────────────────────────────

app.get('/api/users/sessions', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
      'SELECT id, user_agent, ip_address, created_at FROM sessions WHERE user_id = $1 ORDER BY created_at DESC',
      [req.user!.id]
    );
...
app.delete('/api/users/sessions/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
...
app.get('/api/users/search', authenticateToken, async (req: AuthRequest, res: Response) => {
...
app.get('/api/contacts', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  } catch (err) { console.error('contacts error:', err); res.status(500).json({ error: 'Failed to load contacts' }); }
});

app.post('/api/contacts/request', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM contacts WHERE (user_id=$1 AND friend_id=$2) OR (user_id=$2 AND friend_id=$1)',
      [req.user!.id, friendId]
    );
    if (existing.rows.length) return res.status(400).json({ error: 'Contact already exists' });

    const result = await pool.query(
      'INSERT INTO contacts (user_id, friend_id, status, request_message) VALUES ($1,$2,$3,$4) RETURNING id',
  ...
  } catch (err) { console.error('contact request error:', err); res.status(500).json({ error: 'Failed to send request' }); }
});

app.put('/api/contacts/request/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const { status } = req.body;
  if (!['accepted', 'rejected'].includes(status)) return res.status(400).json({ error: 'Invalid status' });
  try {
    const contact = await pool.query(
      'SELECT * FROM contacts WHERE id = $1 AND friend_id = $2 AND status = $3',
      [req.params.id, req.user!.id, 'pending']
    );
    if (!contact.rows.length) return res.status(404).json({ error: 'Request not found' });
    ...
    res.json({ ok: true });
  } catch (err) { console.error('contact update error:', err); res.status(500).json({ error: 'Failed to update request' }); }
});

// ─── Blocks ───────────────────────────────────────────────────

app.post('/api/blocks', authenticateToken, async (req: AuthRequest, res: Response) => {
  const { userId: blockId } = req.body;
  if (!blockId) return res.status(400).json({ error: 'userId required' });
  try {
    await pool.query(
    ...
    res.json({ ok: true });
  } catch (err) { console.error('block error:', err); res.status(500).json({ error: 'Failed to block user' }); }
});
...
app.get('/api/rooms', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  } catch (err) { console.error('rooms list error:', err); res.status(500).json({ error: 'Failed to load rooms' }); }
});

app.get('/api/my-rooms', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const result = await pool.query(
    ...
    try {
      const result = await pool.query(
        'SELECT r.*, rm.role AS user_role, 0 AS unread_count FROM rooms r JOIN room_members rm ON r.id=rm.room_id WHERE rm.user_id=$1 ORDER BY r.name',
        [req.user!.id]
      );
      res.json({ rooms: result.rows });
    } catch (err) { console.error('my-rooms error:', err); res.status(500).json({ error: 'Failed to load rooms' }); }
  }
});

app.post('/api/rooms', authenticateToken, async (req: AuthRequest, res: Response) => {
...
app.get('/api/rooms/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    const roomResult = await pool.query(
      `SELECT r.*, u.username AS owner_username,
        (SELECT role FROM room_members WHERE room_id=r.id AND user_id=$2) AS user_role
       FROM rooms r LEFT JOIN users u ON r.owner_id=u.id WHERE r.id=$1`,
      [roomId, req.user!.id]
    );
    ...
    // check ban
    const ban = await pool.query('SELECT id FROM room_bans WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (ban.rows.length) return res.status(403).json({ error: 'You are banned from this room' });

    const members = await pool.query(
      'SELECT u.id, u.username, u.avatar_color, rm.role FROM room_members rm JOIN users u ON rm.user_id=u.id WHERE rm.room_id=$1 ORDER BY rm.role DESC, u.username',
      [roomId]
    );
    res.json({ room, members: members.rows });
  } catch (err) { console.error('get room error:', err); res.status(500).json({ error: 'Failed to load room' }); }
});

app.put('/api/rooms/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const { name, description } = req.body;
  try {
    const mem = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!mem.rows.length || mem.rows[0].role !== 'admin') return res.status(403).json({ error: 'Admin access required' });
...
app.delete('/api/rooms/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    ...
    if (room.rows[0].owner_id !== req.user!.id) return res.status(403).json({ error: 'Only the owner can delete this room' });
    ...
    res.json({ ok: true });
  } catch (err) { console.error('delete room error:', err); res.status(500).json({ error: 'Failed to delete room' }); }
});

app.post('/api/rooms/:id/join', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    const room = await pool.query('SELECT * FROM rooms WHERE id=$1', [roomId]);
    if (!room.rows.length) return res.status(404).json({ error: 'Room not found' });
    if (room.rows[0].visibility !== 'public') return res.status(403).json({ error: 'This room is private' });

    const ban = await pool.query('SELECT id FROM room_bans WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (ban.rows.length) return res.status(403).json({ error: 'You are banned from this room' });

    await pool.query(
      'INSERT OR IGNORE INTO room_members (room_id, user_id, role) VALUES ($1,$2,$3)',
    ...
    res.json({ ok: true });
  } catch (err) { console.error('join room error:', err); res.status(500).json({ error: 'Failed to join room' }); }
});

app.post('/api/rooms/:id/invite', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const { userId: inviteeId } = req.body;
  try {
    const mem = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!mem.rows.length) return res.status(403).json({ error: 'You are not a member of this room' });

    const room = await pool.query('SELECT name FROM rooms WHERE id=$1', [roomId]);
    await pool.query(
      'INSERT OR IGNORE INTO room_members (room_id, user_id, role) VALUES ($1,$2,$3)',
      [roomId, inviteeId, 'member']
    );
    emitToUser(inviteeId, 'room_invitation', { roomId, roomName: room.rows[0]?.name, invitedBy: req.user!.username });
    res.json({ ok: true });
  } catch (err) { console.error('invite error:', err); res.status(500).json({ error: 'Failed to invite user' }); }
});

app.post('/api/rooms/:id/kick', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId  = parseInt(req.params.id);
  ...
  try {
    ...
    await pool.query('DELETE FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, kickId]);
    emitToUser(kickId, 'room_kicked', { roomId });
    io.to('room_' + roomId).emit('member_left', { userId: kickId });
    res.json({ ok: true });
  } catch (err) { console.error('kick error:', err); res.status(500).json({ error: 'Failed to kick user' }); }
});

app.post('/api/rooms/:id/ban', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId  = parseInt(req.params.id);
  ...
  try {
    ...
    await pool.query('INSERT OR IGNORE INTO room_bans (room_id, user_id, banned_by) VALUES ($1,$2,$3)', [roomId, banId, req.user!.id]);
    await pool.query('DELETE FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, banId]);
    emitToUser(banId, 'room_banned', { roomId });
    io.to('room_' + roomId).emit('member_banned', { userId: banId, bannedBy: req.user!.id });
    res.json({ ok: true });
  } catch (err) { console.error('ban error:', err); res.status(500).json({ error: 'Failed to ban user' }); }
});

app.post('/api/rooms/:id/unban', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const { userId: unbanId } = req.body;
  try {
    const mem = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!mem.rows.length || mem.rows[0].role !== 'admin') return res.status(403).json({ error: 'Admin access required' });
    await pool.query('DELETE FROM room_bans WHERE room_id=$1 AND user_id=$2', [roomId, unbanId]);
    res.json({ ok: true });
  } catch (err) { console.error('unban error:', err); res.status(500).json({ error: 'Failed to unban user' }); }
});

app.get('/api/rooms/:id/bans', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    const mem = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!mem.rows.length || mem.rows[0].role !== 'admin') return res.status(403).json({ error: 'Admin access required' });
    const result = await pool.query(
      'SELECT rb.user_id, u.username, bu.username AS banned_by_username, rb.created_at FROM room_bans rb JOIN users u ON rb.user_id=u.id LEFT JOIN users bu ON rb.banned_by=bu.id WHERE rb.room_id=$1',
      [roomId]
    );
    res.json({ bans: result.rows });
  } catch (err) { console.error('bans error:', err); res.status(500).json({ error: 'Failed to load bans' }); }
});

// ─── Messages ─────────────────────────────────────────────────

app.get('/api/rooms/:id/messages', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const before = req.query.before as string | undefined;
  const limit  = Math.min(parseInt(req.query.limit as string ?? '50'), 100);

  try {
    const mem = await pool.query('SELECT id FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!mem.rows.length) return res.status(403).json({ error: 'Not a member of this room' });

    const params: any[] = [roomId, limit + 1];
    ...
    await pool.query(
      "INSERT INTO room_reads (user_id, room_id, last_read_at) VALUES ($1,$2,strftime('%Y-%m-%d %H:%M:%S','now')) ON CONFLICT(user_id, room_id) DO UPDATE SET last_read_at=excluded.last_read_at",
      [req.user!.id, roomId]
    );

    res.json({ messages, has_more });
  } catch (err) { console.error('messages error:', err); res.status(500).json({ error: 'Failed to load messages' }); }
});

app.put('/api/messages/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const { content } = req.body;
  if (!content?.trim()) return res.status(400).json({ error: 'Content required' });
  try {
    const result = await pool.query(
      "UPDATE messages SET content=$1, edited_at=strftime('%Y-%m-%d %H:%M:%S','now') WHERE id=$2 AND sender_id=$3 AND deleted_at IS NULL RETURNING *",
      [content.trim().slice(0, MAX_MSG_SIZE), req.params.id, req.user!.id]
    );
  ...
  } catch (err) { console.error('edit message error:', err); res.status(500).json({ error: 'Failed to edit message' }); }
});

app.delete('/api/messages/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const msg = await pool.query('SELECT * FROM messages WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
    if (!msg.rows.length) return res.status(404).json({ error: 'Message not found' });

    const m = msg.rows[0];
    const mem = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [m.room_id, req.user!.id]);
    const isAdmin  = mem.rows[0]?.role === 'admin';
    const isOwner  = m.sender_id === req.user!.id;
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Cannot delete this message' });

    await pool.query("UPDATE messages SET deleted_at=strftime('%Y-%m-%d %H:%M:%S','now'), content='[deleted]' WHERE id=$1", [req.params.id]);
...
app.get('/api/messages/personal/:userId', authenticateToken, async (req: AuthRequest, res: Response) => {
  const otherId = parseInt(req.params.userId);
  const before  = req.query.before as string | undefined;
  const limit   = Math.min(parseInt(req.query.limit as string ?? '50'), 100);

  try {
    const params: any[] = [req.user!.id, otherId, limit + 1];
    ...
    await pool.query(
      "INSERT INTO dm_reads (user_id, other_user_id, last_read_at) VALUES ($1,$2,strftime('%Y-%m-%d %H:%M:%S','now')) ON CONFLICT(user_id, other_user_id) DO UPDATE SET last_read_at=excluded.last_read_at",
      [req.user!.id, otherId]
    );

    res.json({ messages, has_more });
  } catch (err) { console.error('personal messages error:', err); res.status(500).json({ error: 'Failed to load messages' }); }
});

app.put('/api/personal-messages/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  const { content } = req.body;
  if (!content?.trim()) return res.status(400).json({ error: 'Content required' });
  try {
    const result = await pool.query(
      "UPDATE personal_messages SET content=$1, edited_at=strftime('%Y-%m-%d %H:%M:%S','now') WHERE id=$2 AND sender_id=$3 AND deleted_at IS NULL RETURNING *",
      [content.trim().slice(0, MAX_MSG_SIZE), req.params.id, req.user!.id]
    );
    if (!result.rows.length) return res.status(404).json({ error: 'Message not found' });
    ...
    res.json({ message: full });
  } catch (err) { console.error('edit personal message error:', err); res.status(500).json({ error: 'Failed to edit message' }); }
});

app.delete('/api/personal-messages/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const msg = await pool.query('SELECT * FROM personal_messages WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
    if (!msg.rows.length) return res.status(404).json({ error: 'Message not found' });
    const m = msg.rows[0];
    if (m.sender_id !== req.user!.id) return res.status(403).json({ error: 'Cannot delete this message' });
    await pool.query("UPDATE personal_messages SET deleted_at=strftime('%Y-%m-%d %H:%M:%S','now'), content='[deleted]' WHERE id=$1", [req.params.id]);
    emitToUser(m.sender_id,   'personal_message_deleted', { id: m.id });
    emitToUser(m.receiver_id, 'personal_message_deleted', { id: m.id });
    res.json({ ok: true });
  } catch (err) { console.error('delete personal message error:', err); res.status(500).json({ error: 'Failed to delete message' }); }
});
...
app.post('/api/upload', authenticateToken, upload.single('file'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [req.user!.id, req.file.filename, req.file.originalname, req.file.size, req.file.mimetype, is_image]
    );
...
app.post('/api/rooms/:id/leave', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    const room = await pool.query('SELECT owner_id FROM rooms WHERE id=$1', [roomId]);
    if (!room.rows.length) return res.status(404).json({ error: 'Room not found' });
    ...
    res.json({ ok: true });
  } catch (err) { console.error('leave room error:', err); res.status(500).json({ error: 'Failed to leave room' }); }
});

// ─── Admin Management ─────────────────────────────────────────

// Get list of room admins
app.get('/api/rooms/:id/admins', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    ...
    const admins = await pool.query(
      'SELECT rm.user_id, u.username, u.avatar_color FROM room_members rm JOIN users u ON rm.user_id = u.id WHERE rm.room_id = $1 AND rm.role = $2',
      [roomId, 'admin']
    );
    res.json({ admins: admins.rows });
  } catch (err) { console.error('get admins error:', err); res.status(500).json({ error: 'Failed to get admins' }); }
});

// Get room members
app.get('/api/rooms/:id/members', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  try {
    const room = await pool.query('SELECT id FROM rooms WHERE id=$1', [roomId]);
    if (!room.rows.length) return res.status(404).json({ error: 'Room not found' });
  ...
  } catch (err) { console.error('get members error:', err); res.status(500).json({ error: 'Failed to get members' }); }
});

// Remove admin (demote to member)
app.post('/api/rooms/:id/remove-admin', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  ...
  try {
    ...
    const member = await pool.query('SELECT role FROM room_members WHERE room_id=$1 AND user_id=$2', [roomId, req.user!.id]);
    if (!member.rows.length) return res.status(403).json({ error: 'You are not a member of this room' });
    
    const isOwner = room.rows[0].owner_id === req.user!.id;
    const isAdmin = member.rows[0].role === 'admin';
    
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Admin access required' });
    if (room.rows[0].owner_id === targetId) return res.status(400).json({ error: 'Cannot remove the owner' });

    await pool.query('UPDATE room_members SET role=$1 WHERE room_id=$2 AND user_id=$3', ['member', roomId, targetId]);
    res.json({ ok: true });
  } catch (err) { console.error('remove admin error:', err); res.status(500).json({ error: 'Failed to remove admin' }); }
});

app.post('/api/rooms/:id/promote', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const { userId: targetId } = req.body;
  try {
    const room = await pool.query('SELECT owner_id FROM rooms WHERE id=$1', [roomId]);
    ...
    await pool.query('UPDATE room_members SET role=$1 WHERE room_id=$2 AND user_id=$3', ['admin', roomId, targetId]);
    res.json({ ok: true });
  } catch (err) { console.error('promote error:', err); res.status(500).json({ error: 'Failed to promote user' }); }
});

app.post('/api/rooms/:id/demote', authenticateToken, async (req: AuthRequest, res: Response) => {
  const roomId = parseInt(req.params.id);
  const { userId: targetId } = req.body;
  try {
    const room = await pool.query('SELECT owner_id FROM rooms WHERE id=$1', [roomId]);
    ...
    await pool.query('UPDATE room_members SET role=$1 WHERE room_id=$2 AND user_id=$3', ['member', roomId, targetId]);
    res.json({ ok: true });
  } catch (err) { console.error('demote error:', err); res.status(500).json({ error: 'Failed to demote user' }); }
});

// ─── Remove Friend ────────────────────────────────────────────

app.delete('/api/contacts/:userId', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    res.json({ ok: true });
  } catch (err) { console.error('remove friend error:', err); res.status(500).json({ error: 'Failed to remove friend' }); }
});

// ─── Protected File Access ────────────────────────────────────

app.get('/uploads/:filename', async (req: AuthRequest, res: Response) => {
  ...
  } catch (err) { console.error('file access error:', err); res.status(500).json({ error: 'Failed to serve file' }); }
});
